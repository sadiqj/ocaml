...
[%%expect{|
{
 ("Falias", module) -> Abs<.4>(X/285, X/285<.3>);
 }
...
[%%expect{|
{
 ("Finclude", module) ->
     Abs<.6>
        (X/289,
         {
          ("t", type) -> X/289<.5> . "t"[type];
          ("x", value) -> X/289<.5> . "x"[value];
          });
 }
...
[%%expect{|
{
 ("Big_to_small1", module) ->
     Abs<.40>
        (shape-var/392,
         {<<internal>>
          ("t", type) -> shape-var/392<<internal>> . "t"[type];
          });
 }
...
[%%expect{|
{
 ("Big_to_small2", module) ->
     Abs<.42>
        (shape-var/398,
         {
          ("t", type) -> (shape-var/398<<internal>> . "t"[type])<.41>;
          });
 }
